...
export default function Gpt() {
  ...
  const gptId = searchParams.get("gptid") || null;
  ...
  useEffect(() => {
    const id = chatId ? chatId : gptId + "_" + chatUid;
    mutate(`/api/get-usage-by-id/${id}`);
  }, [isStreaming]);
    ...
    <div className="h-screen flex flex-row">
      ...
      <div className="h-screen h-full flex flex-col w-full">
        <div className="overflow-y-scroll" id="scroller">
          <Header settings={settings} setSettings={setSettings} gpt={gpt} />
          <Chat
            messages={messages}
            isLoading={isLoading}
